...
public class Main {
    public static void main(String[] args) {
        ...
        //Задача id 1
        Task takeExams = new Task(inMemoryTaskManager.nextId, "Сдать Экзамены",
                "сдать экзамены на 5", Status.NEW);
        inMemoryTaskManager.addSimpleTask(takeExams);

        //Задача id 2
        Task pickUpTheChild = new Task(inMemoryTaskManager.nextId, "Зaбрать ребенка",
        ...
        inMemoryTaskManager.updateSimpleTaskById(2, new Task(1, "Task", "Update Task", Status.NEW));
        //Получаем список всех задач, смотрим изменения
        System.out.println("Список всех задач: " + inMemoryTaskManager.getAllSimpleTasks());


        //Эпик id 3
        Epic buyProducts = new Epic(inMemoryTaskManager.nextId, "Купить продукты", "Description", Status.NEW);
        ...
        //Эпик id 6
        Epic doTheLessons = new Epic(inMemoryTaskManager.nextId, "Сделать уроки", "Description", Status.NEW);
